...
class MenuModel{
    ...
    /**
     * 根据当前登录用户的uid来展示首页左侧树形菜单加载数据
     * @param $uid表示当前登录用户的主键id    
...
//         $sql = "select m.menuid,m.name,m.url,m2.name as parentName,m.isshow from menu m,menu m2 where m.parentid=m2.menuid limit ?,?";
    ...
    public function saveOrUpdateUser($userName, $userPass, $userType, $trueName, $sex,$birthDay,$phone,$school,$education,$workYear,$status, $uid=0){
        $b = true;
        if($uid == 0){
            //无主键值  新增
            $sql  = "insert into menu(userName, userPass, userType, trueName, sex,birthDay,phone,school,education,workYear,status) values(?,?,?,?,?,?,?,?,?,?,?)";
            $b = $this->dbUtil->executeDML($sql, array($userName, $userPass, $userType, $trueName, $sex,$birthDay,$phone,$school,$education,$workYear,$status));
        }else{
            //有主键值 修改
            $sql  = "update menu set userName=?, userPass=?, userType=?, trueName=?, sex=?,birthDay=?,phone=?,school=?,education=?,workYear=?,status=? where uid=?";
            $b = $this->dbUtil->executeDML($sql, array($userName, $userPass, $userType, $trueName, $sex,$birthDay,$phone,$school,$education,$workYear,$status, $uid));
        }
        return $b;
    }
    ...
    public function users(){
        ...
        $a = $this->dbUtil->executeQuery($sql);
        return $a;
    }
